...
function SectionBlock({ id, number, sectionLabel, title, description, primaryContent, children, action, className = '' }) {
  return (
    <section id={id} data-section-component={`${id}Section`} className={`relative h-screen w-screen snap-start snap-always ${className}`}>
      <div className="mx-auto grid h-full w-[88vw] grid-cols-1 grid-rows-[auto_1fr] items-start gap-6 pb-[92px] pt-[112px] lg:w-[89vw] lg:grid-cols-2 lg:grid-rows-1 lg:gap-[clamp(36px,7vw,124px)] lg:pb-5 lg:pt-[108px]">
        {primaryContent || <PrimaryContentPanel sectionId={id} number={number} sectionLabel={sectionLabel} title={title} description={description} action={action} />}
        <InteractionPanel sectionLabel={sectionLabel}>{children}</InteractionPanel>
      </div>
